...
const PastEvents = () => {
  return (
    <Container>
      <h2 className='section_title 2xl:text-7xl 2xl:font-bold'>Past Event Highlights</h2>
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 my-10'>
        {pastEvents?.map((data, index) => (
          <div className='rounded-[20px] custom_border custom_shadow bg-white overflow-hidden'>
            <div key={data.id} className='relative flex items-center justify-center w-full h-[300px]'>
              ...
              <div className='absolute flex items-end pl-6 pb-4 top-0 left-0 size-full bg-[linear-gradient(0deg,_rgba(0,0,0,0.5)_0%,_rgba(0,0,0,0.5)_100%)]'>
                  <div className='space-y-2'>
                    ...
                    <p className='text-primary-gray text-xl tracking-wider'>{data.description}</p>
                </div>
              </div>
            </div>
