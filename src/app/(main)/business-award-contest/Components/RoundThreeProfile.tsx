...
import { CMSBusinessAward, CMSHomepage, CMSEventsPage } from "@/Types/cms";
import {
  getBossCms,
  getCMSAboutData,
  getCMSHomepageData,
...
export default function RoundThreeProfile({
  ...
  const [bossData, setBossData] = useState<CMSBusinessAward | null>(null);
    ...
    async function loadData() {
      try {
        const [bossCms, aboutCms, homepageCms, eventsCms] = await Promise.all([
          getBossCms() as Promise<CMSBusinessAward>,
          getCMSAboutData(),
  ...
  return (
    <>
      ...
      <RoundTwoAbout contestant={contestant} roundNumber={3} />
      <Roundhero
