    ...
    <section className='section'>
      <Container>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mt-12'>
          {data?.map((data, index) => (
            <div key={index} className='flex flex-col px-7 py-8 gap-7 rounded-2xl custom_border bg-white'>
              <div className='size-[100px] flex items-center justify-center rounded-full shrink-0 bg-[#1977DD29]'>
                <data.icon />
              </div>
              <div className='space-y-3'>
                <h5 className='text-primary-black text-[32px] font-semibold'>{data.title}</h5>
