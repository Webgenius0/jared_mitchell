...
const DiscoverArtists = ({ type = 'artist' }: { type?: 'artist' | 'business' }) => {
  ...
  return (
    <section className='section'>
      <Container>
        <h2 className='section_title'>Discover More {type === 'artist' ? 'Artists' : 'Businesses'}</h2>
        <p className='section_sub_title'>Meet the {type === 'artist' ? 'creatives' : 'businesses'} shaping our neighborhoods.<br />
        ...
        {isLoading ? (
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mt-12'>
            {data?.map((item: any, index: number) => (
              <div key={item.id || index} className='p-5 rounded-xl custom_border custom_shadow bg-white space-y-4'>
